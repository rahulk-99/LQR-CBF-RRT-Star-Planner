"""
This code implements Control Barrier Function (CBF) based Rapidly-exploring Random Tree (RRT) 
planning algorithm for safe motion planning with obstacle avoidance.

The implementation includes:
1. CBF-based safety constraints for different system models (linear, unicycle)
2. Quadratic Programming (QP) based control synthesis
3. Motion planning with safety guarantees
4. Support for different control models (velocity and acceleration control)
...
class Obstacle_Sphere(object):
    """
    Class representing spherical obstacles in the environment.
    Implements barrier functions and their derivatives for obstacle avoidance.
    """
    def __init__(self, center, radius):
        self.T = 1.0  # Integration Length
        self.N = 50  # Number of Control Updates
        self.center = center  # Center coordinates of the obstacle
        self.radius = radius  # Radius of the obstacle

    def h(self, x):
        """Barrier function: h(x) = ||x - center||^2 - radius^2"""
        return LA.norm(x - self.center, 2) ** 2 - self.radius**2

    def gradh(self, x):
        """Gradient of barrier function: ∇h(x) = 2(x - center)"""
        return 2 * (x - self.center)

    def hdot(self, x, xdot):
        """Time derivative of barrier function: ḣ(x) = ∇h(x)·ẋ"""
...
class CBF_RRT:
    """
    Main class implementing CBF-RRT algorithm for safe motion planning.
    Combines RRT with Control Barrier Functions for guaranteed safety.
    """
    def __init__(self, obstacle_list):
        ...
        # CBF parameters
        self.k = 6  # k nearest neighbor obstacles for CBF constraint generation
        self.cbf_constraints_sensing_radius = 500  # Sensing radius for CBF constraints
        self.k_cbf = 80.0  # CBF coefficient for double integrators
        self.k_cbf2 = 3.0  # CBF coefficient for acceleration control
        self.p_cbf = 15  # CBF constraint power
        self.k1_unicyle_cbf = 2.0  # CBF coefficient for unicycle model
        self.k2_unicyle_cbf = 2.0  # CBF coefficient for unicycle model
    ...
    def CLF_unicycle_QP(self, x_current, x_goal):
        """
        Control Lyapunov Function (CLF) based Quadratic Programming for unicycle model.
        Solves QP to find control inputs that stabilize the system while respecting constraints.
        
        Args:
            x_current: Current state [x, y, theta]
            x_goal: Goal state [x, y]
            
        Returns:
            u_ref: Optimal control input (angular velocity)
        """
    ...
    def QP_controller(self, x_current, u_ref, model="linear"):
        """
        Quadratic Programming based controller that enforces CBF constraints.
        
        Args:
            x_current: Current state
            u_ref: Reference control input
            model: System model type ("linear" or "unicycle")
            
        Returns:
            Optimal control input that satisfies CBF constraints
        """
        if model == "linear":
    ...
    # Add a clearance by inflating the obstacle radius:
                safe_margin = 100  # adjust based on your safety requirements
                print("safe_margin", safe_margin)
                inflated_radius = self.x_obstacle[i][2] + safe_margin
                h = (
                ...
                # Here you can also add the safe_margin inside the constraint if needed:
    ...
    def find_knn_obstacle(self, x_current, x_obstacles, k):
        """
        Find k nearest obstacles to the current state using priority queue.
        
        Args:
            x_current: Current state
            x_obstacles: List of obstacles
            k: Number of nearest neighbors to find
            
        Returns:
            List of indices of k nearest obstacles
        """
    ...
    def find_obstacles_within_cbf_sensing_range(self, x_current, x_obstacles):
        """
        Find obstacles within the CBF sensing radius of the current state.
        ...
        Returns:
            List of indices of obstacles within sensing range
        """
        obstacles_idx = []
    ...
    def motion_planning_with_QP(self, u_ref, model="linear_velocity_control"):
        """
        Motion planning using QP-based control synthesis.
        ...
        Returns:
            Tuple of (state trajectory, control trajectory)
        """
        x_current = self.y0
        x = np.zeros((2, 0))
        u = np.zeros((2, 0))
    ...
    def QP_constraint(self, x_current, u_ref, system_type="linear_velocity_control"):
        """
        Check if the control input satisfies CBF constraints for the given system type.
        
        Args:
            x_current: Current state
            u_ref: Reference control input
            system_type: Type of system model
    ...
    def motion_planning_without_QP(self, u_ref, model="linear_velocity_control"):
        """
        Motion planning without QP-based control synthesis.
        Uses direct integration of system dynamics.
    ...
    def plot_traj(self, x, u):
        """
        Plot the generated trajectory and obstacles.
...
if __name__ == "__main__":
    ...
    # Create CBF-RRT planner instance
    CBFRRT_Planning = CBF_RRT(initial_state, obstacle_list)
    
    # Generate trajectory using QP-based motion planning
    x, u = CBFRRT_Planning.motion_planning_with_QP(u_ref, model="linear_velocity_control")
